...
// export const fetchAgentData = async (agentCode) => {
//   if (!agentCode) throw new Error("Agent code missing!");

//   try {
//     console.log("📡 Fetching data for agent:", agentCode);

//     // 1️⃣ Fetch Agent Info
//     let agentData = null;
//     try {
//       const res = await axios.get`(${BASE_URL}/masters/agents/${agentCode})`;
//       agentData = res.data?.data || {};
//     } catch {
//       console.warn("⚠ No agent found for code:", agentCode);
//     }

//     const agentInfo = {
//       AgentCode: agentData?.AgentCode || agentCode,
...
//     const orderRes = await axios.post($`{BASE_URL}`/orders/by-agent, {
//       agentCode,
//     });
//     const backendOrders = orderRes.data?.data || [];

//     if (backendOrders.length === 0) {
//       agentCache[agentCode] = { agentInfo, orders: [] };
//       return { agentInfo, orders: [] };
//     }

//     // 3️⃣ Fetch Route Info
//     const firstRoute = backendOrders[0]?.route;
//     if (firstRoute) {
//       try {
//         const routeRes = await axios.get(
//           $`{BASE_URL}/masters/routes/${firstRoute}`
//         );
//         const routeData = routeRes.data?.data;
//         agentInfo.RouteName =
//           routeData?.RouteNameEng || routeData?.RouteName || "Unknown Route";
//         agentInfo.VehichleNo = routeData?.VehichleNo || "N/A";
//       } catch {
//         console.warn("⚠ Failed to fetch route info");
//       }
//     }

//     // 4️⃣ Return one entry per order with item details preserved
//     const ordersWithSummary = backendOrders.map((order) => {
//       const items = order.itemInfo || [];
...
//         itemInfo: items, // 👈 keep full details here
...
//     // agentInfo.TotalOrder = ordersWithSummary
//     //   .reduce((sum, o) => sum + (o.totalPrice || 0), 0)
//     //   .toFixed(2);

//     agentCache[agentCode] = { agentInfo, orders: ordersWithSummary };
//     console.log("✅ Orders with summary:", ordersWithSummary);

//     return { agentInfo, orders: ordersWithSummary };
//   } catch (err) {
...
// /* ----- Configuration ----- */
// const BASE_URL = "http://192.168.1.11:8001/api/orders";
...
//       let agentData = {};
//       try {
//         const res = await apiClient.get(`/masters/agents/${encodeURIComponent(key)}`);
//         agentData = res.data?.data || {};
//       } catch (err) {
